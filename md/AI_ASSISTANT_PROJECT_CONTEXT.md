...
| ループのプリロール/ポストロール中に前/次マーカーボタンが1つ飛ばす | 3-22, 3-1 |
| SPでBackupの「含める項目」を開くと、リストの途中に重なって表示される | 3-23 |
| 無料版ロック中のマーカー行だけ表示が崩れる（文字が右寄り等） | 3-24 |
| 無料版制限ありでも、下部の曲送り/曲戻しで制限を超えて聴けてしまう | 3-25, 3-9 |
| SPでBackupの曲選択リストが広すぎて、含める項目までスクロールしづらい | 3-26, 3-23 |
...
### 3-22. 前/次マーカーボタンが、プリ/ポストロール中は区間を1つ飛ばす
- **現象：** 1-2間をループ中（プリロール5秒）、2を通過したポストロール中に
  「次のマーカー」を押すと、2-3ではなく3へ飛んでしまう。
- **原因：** `jumpToNextMarker()`/`jumpToPrevMarker()`が、単純に再生位置
  （`audio.currentTime`）だけを基準にしていた。ポストロール中は位置的には
  2より後ろにいるため、「次」は3になっていた。
- **解決（v2.16.6）：** `getMarkerNavReferenceTime()`を追加。マーカーループ中に
  プリロール/ポストロール部分を再生している間は、今ループ中の区間
  （`loopActiveMarkerIndex`）の内側にいるものとして基準時刻を補正する
  （ポストロール中＝区間の終わりの少し手前、プリロール中＝区間の頭）。
  「次」は次の区間の頭へ、「前」は今の区間の頭へ移動する。
- **教訓：** 「今どこにいるか」を使う操作（前/次・区間表示等）は、再生位置
  そのものではなく「ユーザーが今聴いている区間」を基準にすべき場面がある。
  プリ/ポストロールに関わる新機能では、`loopActiveMarkerIndex`との整合を確認する。

### 3-23. overflow-y:autoの中でflex:1を何段にも重ねると、iOSで内容が重なる
- **現象：** SPでBackupパネルの「含める項目」を展開すると、見出しや項目行が
  曲リストの途中に重なって表示される（PCでは症状が出ない）。
- **原因：** `#pcV2PanelBody`はパネル共通の基本ルールで元々
  `overflow-y: auto`（それ自体がスクロールする要素）になっている。
  v2.15.1のBackupレイアウトは、そこへさらに`display: flex; flex-direction:
  column`を足し、`.export-modal-body`→`.export-section:first-child`→
  `.track-backup-tracklist`と3段階で`flex: 1; min-height: 0`を重ねて
  「曲リストが残り高さを全部もらう」構成にしていた。「overflow-y: autoの
  要素の中でflex:1を多段に重ねて高さを埋めさせる」構成は、iOS Safariで
  リストの内容量に応じた高さ確定のタイミングによって、後続セクションが
  古い（確定前の）位置のまま描画され重なって見えることがある既知の
  相性問題。Chromeでは再現しなかったため、PCの動作確認だけでは
  気づけなかった。
- **解決（v2.16.7）：** flex:1で「埋める」構成をやめ、曲選択リストは
  `max-height: min(58vh, 520px)`という上限つきの固定的な高さにして
  自身の中だけで`overflow-y: auto`させる（元のCSS、`.track-backup-tracklist`
  基本ルールの`max-height: 220px`を広げただけ）。パネル全体は他のパネルと
  同じく`#pcV2PanelBody`の1箇所だけでスクロールする、シンプルな構成に戻した。
- **教訓：** **`overflow-y: auto`（または`scroll`）を持つ要素の内部で、
  「残り高さを埋める」ためのflex:1チェーンを何段にも重ねない。** 特に
  SP・iOS Safariでの見た目は、Chromeでの確認だけでは保証されない。
  「区画ごとに高さを分配する」より「一番かさばる部分にだけ上限つきの
  固定的な高さを与えて自前でスクロールさせる」方が、こうした環境差の
  影響を受けにくく安全。新しいレイアウトを組む時、複数のflex:1が入れ子に
  なっていないか、その外側がoverflow:autoでないかを確認する。

### 3-24. display:gridの固定列数に対し、条件付きで子要素の数を変えてしまう
- **現象：** 無料版のマーカーロック行だけ、色の丸と時刻の文字の間が
  大きく空いて文字が右寄りに表示され、削除チェックが見当たらなくなる
  （EDITモードで数えると行の高さも他よりわずかに大きい）。
- **原因：** `.pinItem`は`display: grid; grid-template-columns: auto 1fr
  auto auto;`という「4列固定」のグリッドで、`renderPinList()`が
  colorMark・labelRow・toggleBtn・delZoneの4つを順にappendする前提で
  組まれていた。ロック中の行だけ、鍵アイコン(`lockIcon`)をcolorMarkの
  前に**別の子要素として**appendしていたため子要素が5個になり、
  5個目(delZone)が4列を使い切った次の暗黙の行へ折り返され、結果として
  鍵→1列目、色の丸→2列目(1fr、本来ラベル用の広い列)、ラベル→3列目(auto)、
  目→4列目、という具合に全部が1列ずつズレていた。
- **解決（v2.16.8）：** 鍵アイコンを独立した子要素にせず、colorMarkと
  一緒に`.pin-leading-cell`という1つの入れ物（flexで横並び）にまとめ、
  `.pinItem`の子要素が常に4個になるようにした。
- **教訓：** **`display: grid`に固定の`grid-template-columns`を使っている
  親へ、条件によって子要素の「個数」を変える変更を加えない。** 増減させたい
  時は、既存の列に収まる入れ物（wrapper）の中身を増減させる。個数が
  変わりうるgrid/flexは、なるべく`grid-template-columns: repeat(...)`や
  明示的な`grid-column`指定にしておくと、こうしたズレに気づきやすい。

### 3-25. 制限のチェックが「入り口」の一部にしかなく、他の経路から素通りできる
- **現象：** 無料版のライブラリ曲数制限（例：3曲まで）を超えて曲があっても、
  下部コントローラーの「次の曲」「前の曲」ボタンを押し続けると、制限を
  超えた曲まで普通に聴けてしまう。
- **原因：** 曲を再生する経路は実際には複数ある（ライブラリの行をタップ、
  サムネイルをタップ、下部の前/次ボタン、ロック画面/通知のメディア
  コントロール、曲が終わった時の自動送り）が、どれも最終的には
  `playTrackAt(index)`を呼ぶ一方で、シェアウェア制限のチェックは
  「行をタップした時」「サムネイルをタップした時」の2箇所にしか
  書かれていなかった。前/次ボタン（`playPrevTrack`/`playNextTrack`）・
  メディアコントロール・自動送りは、このチェックを経由せず
  `playTrackAt()`を直接呼んでいたため、素通りできていた。
- **解決（v2.16.8）：** `playTrackAt()`自身の先頭に同じチェックを追加し、
  呼び出し元に関わらず一律にブロックされるようにした（最終防衛ライン）。
  個別の呼び出し元にあった重複チェックは、早期リターンで
  `playTrackAt()`まで到達しないため無害（二重にトーストが出ることもない）。
- **教訓：** **アクセス制限は「みんなが通る一番奥の関数」に置く。**
  入口（クリックハンドラ等）ごとに個別にチェックを書くと、新しい入口
  （ボタン・ショートカット・自動処理・外部API等）を追加するたびに
  チェックを足し忘れるリスクがある。新しい再生経路・操作経路を追加する
  時は、その経路が制限チェックを経由する関数を通るか必ず確認する。

### 3-26. 内側スクロール領域が大きすぎると、外側へのスクロールの受け渡しがシビアになる
- **現象：** SPのBackupパネルで、曲数が多いと曲選択リスト（内側で
  独自にスクロールする領域）がほぼ画面いっぱいになり、その下にある
  「含める項目」やDOWNLOADへスクロールしようとしても、指がリストの上に
  乗っている間はリスト自身のスクロールが優先され続け、外側（パネル
  全体）のスクロールへなかなか切り替わらない。指を一度離してリストの
  外側から再度スワイプすれば届くが、操作として分かりにくい。
- **原因：** §3-23でBackupパネルを「flex:1で高さを埋める」構成から
  「曲選択リストだけ上限つきの高さで自前スクロール」する構成に直した際、
  その上限を`min(58vh, 520px)`という比較的大きな値にしていた。曲数が
  多いと、この内側スクロール領域だけで画面のほとんどを占めてしまい、
  外側への「受け渡し」に必要な操作（指を離して外側から振り直す等）が
  頻繁に発生していた。
- **解決（v2.16.9）：** 上限を`min(38vh, 340px)`まで縮小。内側の
  スクロール領域を画面の一部にとどめることで、下の「含める項目」に
  スクロールなしで（または、あっても少しのスクロールで）届くようにした。
- **教訓：** **内側に独自スクロールを持つ領域は、外側の要素へ指を離さず
  スクロールし続けたい場面がある時、画面を埋め尽くすほど大きくしない。**
  「もっと広く」という要望であっても、その領域の下に他の操作（別セクション・
  確定ボタン等）が続く設計では、上限を控えめにして全体の見通しと
  操作の両方を確保する方がよい。
